...
def infer_tau_dr(x: Iterable[float], t_obs: int, y_obs: float) -> Tuple[float, float]:
    t0 = time.perf_counter_ns()
    M = _require_model()
    X = _to_x_array(x)
    e = float(np.clip(M['prop'].predict_proba(X)[0, 1], 0.001, 1 - 0.001))
    mu0 = float(M['mu0'].predict(X)[0])
    mu1 = float(M['mu1'].predict(X)[0])
    t_obs = int(t_obs)
    y_obs = float(y_obs)
    tau = mu1 - mu0 + t_obs * (y_obs - mu1) / e - (1 - t_obs) * (y_obs - mu0) / (1 - e)
    us = (time.perf_counter_ns() - t0) / 1000.0
    return (float(tau), us)
